...
class TrackerViewsTestCase(TestCase):
    ...
    def setUp(self):
        test_user1 = User.objects.create_user('test_user1',
                                              'test@example.com',
                                              'testpass1')
        test_user2 = User.objects.create_user('test_user2',
                                              'test2@example.com',
                                              'testpass2')
        self.test_bench1 = BenchMovement.objects.\
            create(user=test_user1, created_at=timezone.now(),
                   bar_type="dive", floor=True,
                   reverse=True, board=0, manpon=False,
                   pin=False, bench_notes="None", bands_type="Some",
                   chain_weight=300, movement_weight=300, movement_sets=3,
                   movement_reps=2, media_url="None")
        self.test_bench1.save()
        self.test_bench2 = BenchMovement.objects.\
            create(user=test_user2, created_at=timezone.now(),
                   bar_type="classy", floor=True, reverse=True,
                   board=False, manpon=False, pin=False,
                   bench_notes="None", bands_type="Some",
                   chain_weight=300)
...
class SquatMovementTestCase(TestCase):
    def setUp(self):
        test_user = User.objects.create_user('test',
                                             'test@example.com',
                                             'testpass')
        SquatMovement.objects.create(user=test_user,
